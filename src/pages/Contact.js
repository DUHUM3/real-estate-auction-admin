...
const ContactsPage = () => {
  ...
  const fetchContacts = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/admin/contacts?${queryParams}`, {
        headers: {
  ...
  }, [filters.start_date, filters.end_date, filters.sort_by, filters.sort_order, filters.per_page]);
  ...
  const handleMarkAsContacted = async (id) => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(`${API_BASE_URL}/api/admin/${id}/mark-contacted`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      'غير مقروءة': { color: 'bg-yellow-100 text-yellow-800', label: 'غير مقروءة' },
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">عدد العناصر في الصفحة</label>
            <select
        ...
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-600">
            عرض {paginationInfo.start} إلى {paginationInfo.end} من أصل {pagination.total} رسالة
          </div>
                ...
                contacts.map((contact) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                      <div className="flex justify-start space-x-2 space-x-reverse" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => handleViewDetails(contact.id)}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{selectedContact.phone || 'غير متوفر'}</p>
                </div>
                <div>
              ...
              {selectedContact.file_path && (
                <div>
                  ...
                  <div className="mt-2">
                    <a
                      href={`${API_BASE_URL}/download-file?path=${encodeURIComponent(selectedContact.file_path)}`}
                      target="_blank"
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{formatDate(selectedContact.updated_at)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
              <div className="flex justify-between items-center">
                <div className="flex space-x-3 space-x-reverse">
                  ...
                  >
                    {selectedContact.status === 'تم التواصل' ? 'تم التواصل ✓' : 'تم التواصل'}
                  </button>
                </div>
                <div className="flex space-x-3 space-x-reverse">
                  <button
